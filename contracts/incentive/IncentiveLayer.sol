...
contract IncentiveLayer is DepositsManager, RewardsManager {
    ...
    mapping(bytes32 => address[]) private taskCandidateSolvers; // Each time a solver is registered for a task, a new entry is added to this mapping.
                                                               // the key is the taskID and the value contains a list with all solver accounts that have registered for the task.
                                                              // as soon as a solver is selected, the corresponding entry is removed from the mapping.
    ...
    function contains(bytes32 taskID, address solverAddress) private view returns (bool) {
        uint arrLength = taskCandidateSolvers[taskID].length;
        for (uint i=0; i<arrLength; i++) {
            if(taskCandidateSolvers[taskID][i] == solverAddress){
                return true;
            }
        }
        return false;
    ...
    // @param taskID – the task id.
    // @return – boolean
    function registerForTask(bytes32 taskID) public returns(bool) {
        Task storage t = tasks[taskID];
        ...
        taskCandidateSolvers[taskID].push(msg.sender);
 
        return true;
    }


    // @dev – solver is manually selected for task (or probably through an auction mechanism)
    // @param taskID – the task id.
    // @param randomBitsHash – hash of random bits to commit to task
    // @return – boolean
    function selectSolverForTask(bytes32 taskID, address _selectedSolver, bytes32 randomBitsHash) public returns(bool) {
        Task storage t = tasks[taskID];
        VMParameters storage vm = vmParams[taskID];
        require(t.selectedSolver == address(0x0));
        require(contains(taskID, _selectedSolver));

        bondDeposit(taskID, _selectedSolver, t.minDeposit);
        t.selectedSolver = _selectedSolver;
        t.randomBitsHash = randomBitsHash;
        t.state = State.SolverSelected;
        t.timeoutBlock = block.number + (1+vm.gasLimit/RUN_RATE);

        delete taskCandidateSolvers[taskID];

        emit SolverSelected(taskID, _selectedSolver, t.initTaskHash, t.minDeposit, t.randomBitsHash);
	return true;
    }
