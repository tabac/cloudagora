...
vorpal
  .command('select', 'selects a solver for a task')
  .option('-a, --account <num>', 'index of web3 account to use.')
  .option('-t, --task <num>', 'the taskID for which the solver is selected.')
  .types({
	  string: ['t', 'task']
	 })
  .action(async (args, callback) => { await cliLib.selectSolver({ os, args }); callback() })

vorpal
  .command('ls tasks', 'lists all active tasks')
  .option('-a, --account <num>', 'the account that has listened to the task creation event.')
  .action(async (args, callback) => { await cliLib.availableTasks({ os, args }); callback() })

vorpal
  .command('register', 'solver registers for task')
  .option('-a, --account <num>', 'the solver account that wants to register.')
  .option('-t, --task <num>', 'the taskID to which we want to register.')
  .types({
	  string: ['t', 'task']
	 })
  .action(async (args, callback) => { await cliLib.register({ os, args }); callback() })
