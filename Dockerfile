...
RUN apt-get  update \
 && apt-get install -y git cmake ninja-build g++ python wget ocaml opam libzarith-ocaml-dev m4 pkg-config zlib1g-dev psmisc sudo curl tmux vim apache2 \
 && opam init -y 
...
RUN npm install -g ganache-cli mocha browserify
...
# ipfs and eth ports
# the 9870 port is for exposing a REST API for a client actor
# the 9999 port is for exposing a REST API for a provider actor
# the 4200 port is for the angular application

EXPOSE 4001 30303 80 8545 9870 9999 4200
